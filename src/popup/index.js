...
async function loadStats() {
  try {
    const response = await chrome.runtime.sendMessage({ action: MessageActions.GET_STATS });
...
async function loadVocabulary() {
  try {
    const response = await chrome.runtime.sendMessage({ action: MessageActions.GET_VOCABULARY });
...
function initEventListeners() {
      ...
      if (confirm('確定要刪除這個單字嗎？')) {
        try {
          await chrome.runtime.sendMessage({
            action: MessageActions.REMOVE_VOCABULARY,
            index: index
          });
  ...
  document.getElementById('clear-btn').addEventListener('click', async () => {
    ...
    if (confirm('確定要清空所有單字嗎？此操作無法復原。')) {
      try {
        await chrome.runtime.sendMessage({ action: MessageActions.CLEAR_VOCABULARY });
        await loadVocabulary();
      } catch (error) {
